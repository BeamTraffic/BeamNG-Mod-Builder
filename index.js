...
if (outputName == "BuilderDefaultName") {
    console.log(
        chalk.red("Skipping build, mod name is the default one, please change it!")
    );
    return;
}
...
output.on("close", async function () {
    ...
    fs.copyFileSync(
        outputPath,
        path.join(outputdirversions, `${outputName}_v${version}.zip`)
    );
    ...
    logLine();
    console.log(
        chalk.blue(`Total Time: ${((endTime - startTime) / 1000).toFixed(2)}s`)
    );
